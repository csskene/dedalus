...
def main(filename, start, count, output):
    ...
    # Layout
    nrows, ncols = len(tasks), N_cardinal
        ...
        for index in range(start, start+count):
            for i, task in enumerate(tasks):
                for j in range(N_cardinal):
                    ...
                    plot_tools.plot_bot(dset, image_axes=image_axes, data_slices=data_slices, axes=axes, title=task, even_scale=True, visible_axes=False)
            # Add time title
