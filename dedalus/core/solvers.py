...
from ..tools.array import scipy_sparse_eigs, slepc_sparse_eigs
...
class EigenvalueSolver(SolverBase):
    ...
    **kw :
    ...
    def solve_sparse(self, subproblem, N, target, rebuild_matrices=False, left=False, normalize_left=True, raise_on_mismatch=True, v0=None, eig_solver='scipy', **kw):
        """
        ...
        eig_solver : 'scipy' or 'slepc', optional
            Which eigenvalue solver to use scipy or slepc (default: scipy)
        **kw :
        ...
        if eig_solver=='scipy':
            ...
            eig_output = slepc_sparse_eigs(A=A, B=B, left=left, N=N, target=target, matsolver=self.matsolver)
        if left:
